...
public final class HybridLUTProcessor: HybridLUTProcessorSpec {
  ...
  public func applyLutToVideo(videoPath: String, lutPath: String, intensity: Double) throws -> Promise<String> {
    let amount = Float(min(max(intensity, 0.0), 1.0))
    return Promise<String>.parallel {
      do {
        return try VideoLUTProcessor.applyLutToVideo(
          videoPath: videoPath,
          lutPath: lutPath,
          intensity: amount
        )
